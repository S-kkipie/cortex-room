...
import type {
    ...
    Participant,
    TranscriptSegment,
} from "../container/src/contract/events";
import { EventBuffer } from "../container/src/emit/buffer";
import {
    createPortalPublisher,
    type Publisher,
} from "../container/src/emit/portal";
import {
    createRecallBot,
    RECALL_EVENTS,
    stopRecallBot,
} from "../container/src/recall/client";
...
export function shouldContinueAlarm(
    state: string,
    hasBridge: boolean,
): boolean {
...
export class MeetingAgent extends DurableObject<Env> {
    ...
    private pub(): Publisher {
        if (!this.publisher) {
            this.publisher = createPortalPublisher({
                apiKey: this.env.PORTAL_API_KEY,
                channelId: `meeting-${this.meetingId}`,
            });
        }
    ...
    private emit(ev: AgentEvent): void {
        if (ev.type === "participant.joined")
            this.participants.set(ev.participant.participantId, ev.participant);
        if (ev.type === "participant.left")
            this.participants.delete(ev.participantId);
        this.buffer.append(ev);
        void this.pub().publish(ev);
        if (this.bridge && bridgeShouldConsume(ev))
            this.bridge.onSegment(
                (ev as { segment: TranscriptSegment }).segment,
            );
    }
    ...
    async fetch(req: Request): Promise<Response> {
        ...
        if (req.method === "POST" && path === "/start") {
            const { meetingUrl, canvasProjectId } = (await req.json()) as {
                meetingUrl?: string;
                canvasProjectId?: string;
            };
            if (!meetingUrl)
                return new Response("meetingUrl required", { status: 400 });
            if (canvasProjectId !== undefined && !isUuid(canvasProjectId)) {
                return new Response("canvasProjectId must be a UUID", {
                    status: 400,
                });
            }
            let botId: string;
            try {
                ...
                return new Response(
                    err instanceof Error ? err.message : "recall create failed",
                    { status: 502 },
                );
            }
            ...
            this.emit({
                ...
                at: new Date(this.t0Ms).toISOString(),
            });
        ...
        if (path === "/stream") {
            ...
            this.buffer.subscribe((ev) => {
                writer
                    .write(enc.encode(`data: ${JSON.stringify(ev)}\n\n`))
                    .catch(() => {});
            });
        ...
        if (req.method === "POST" && path === "/webhook") {
            ...
            if (res.anchorT0Ms != null && this.t0Anchored == null)
                this.t0Anchored = res.anchorT0Ms;
